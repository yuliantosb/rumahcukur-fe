...
import { updateCoupon, getCoupon } from '../../store/actions/couponAction';
...
class EditCoupon extends React.Component {
	...
	handleSubmit = (e) => {
		e.preventDefault();
        this.props.updateCoupon(this.props.match.params.id, this.state);
    }
        ...
        if (nextProps !== this.props) {
            if (nextProps.data) {
                this.setState({
                    ...
                    value: nextProps.data.value ? nextProps.data.value : '',
                    start_period: nextProps.data.start_period ? new Date(nextProps.data.start_period) : '',
                    end_period: nextProps.data.end_period ? new Date(nextProps.data.end_period) : '',
		...
		return (
			...
			<Container fluid className="main-content-container px-4">
                <Loading
						...
						/>
				<Helmet>
					<title>Edit Coupon | {appName} </title>
				</Helmet>
				<Row noGutters className="page-header py-4">
                    <div className="col-md-8">
					    <PageTitle sm="4" title="Edit Coupon" className="text-sm-left" />
                    </div>
                                            ...
                                            <div className="row">
                                                <div className="col-md-6">
                                                    <div className="form-group">
                                                        ...
                                                        <input type="text" id="name" value={ this.state.name } className={`form-control ${ error && error.data.errors.name && 'is-invalid' }`} onChange={this.handleChange} placeholder="Coupon unique name" />
                                                        { 
                                                    ...
                                                    <div className="row">
                                                        <div className="col-md-8">
                                                            <div className="form-group">
                                                                ...
                                                                <NumberFormat value={this.state.value} id="value" decimalSeparator="." thousandSeparator="," className={`form-control text-right ${ error && error.data.errors.value && 'is-invalid' }`} onChange={this.handleChange} placeholder="0.0" />
                                                                { 
                                                        ...
                                                        <div className="col-md-4">
                                                            <div className="form-group">
                                                                ...
                                                                <select value={this.state.type} id="type" className="form-control custom-select" onChange={this.handleChange}>
...
const mapStateToProps = (state) => {
    return {
        ...
        data: state.coupon.coupon.data
    }
}

const mapDispatchToProps = (dispatch) => {
    return {
        updateCoupon: (id, data) => dispatch(updateCoupon(id, data)),
        getCoupon: id => dispatch(getCoupon(id))
    }
}

export default connect(mapStateToProps, mapDispatchToProps)(withToastManager(EditCoupon));
