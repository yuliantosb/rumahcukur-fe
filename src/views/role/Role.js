...
import { fetchRole, deleteRole } from '../../store/actions/roleAction';
...
class Role extends React.Component {
	...
	handleClickYes = () => {
		...
		this.props.deleteRole(this.state.deleteIdRole);
	}
			...
			return (
				<tr key={role.id}>
					<td>
                        ...
                        <Link to={`role/edit/${role.id}`} className="btn btn-link btn-sm py-0 px-0 mr-2 text-success">Edit</Link>
                        <button onClick={() => this.handleClickDelete(role.id)} className="btn btn-link btn-sm px-0 px-0 text-danger">Delete</button>
                    </td>
						...
						<div className="col-md-12 mt-4">
							<div className="row">
								<div className="col-md-8">
									<Link to="/role/create" className="btn btn-secondary"><i className="mdi mdi-plus"></i> Add</Link>
								</div>
...
const mapStateToProps = (state) => {
    return {
		...
		isDeleted: state.role.isDeleted
    }
}
